...
# NOTE: files are Cassava 1.8 formatted nullifying the need for a manifest ...
# ... however, this section also serves to validate ...
...
# (3;4;5) read in sampledata and create sorted vector
...
M.smp_1 <- as.character(sort(M.smp_0$SampleID))

LOGICAL_M.sID <- all(M.mft_unq_1 == M.smp_1)
...
# in reads.qzv:
# Per-sample sequence counts - Total Samples: 273
# Demultiplexed sequence counts summary - Total: 9,121,905

# in R Console run:
Q.S1_TotalSamples <- 273
Q.S1_TotalSequences <- 9121905
...
# in R Console run:
Q.S2_TotalSamples <- 273
Q.S2_TotalSequences <- 9121905
...
# runtime: ~3 hours
# --p-n-reads-learn 1000000 change from default = 10000000
...
# Number of features 3,536
# Total frequency 7,839,387

# in R Console run:
Q.S3_NumberSamples <- 273
Q.S3_NumberFeatures <- 3536
Q.S3_TotalFrequency <- 7839387
